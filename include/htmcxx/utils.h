...
{   
    ...
    { 
        std::string class_with_namespace = abi::__cxa_demangle(typeid(object).name(), 0, 0, 0);

        auto class_name = class_with_namespace  | std::views::take_while([](const char& c){ return c != '<';})
                                                | std::views::reverse 
